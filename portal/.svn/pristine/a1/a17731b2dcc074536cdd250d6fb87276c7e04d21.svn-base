...
public class Dial {
	...
	public Dial() {
		try {
			...
			this.dial.setAttribute("moh", ""
					+ MusicOnHold.NORMAL.name().toLowerCase());
	...
	public Dial(Boolean record) {
		try {
			...
			this.dial.setAttribute("moh", MusicOnHold.NORMAL.name()
					.toLowerCase());
			this.doc.appendChild(this.dial);
		} catch (Exception e) {
			e.printStackTrace();
