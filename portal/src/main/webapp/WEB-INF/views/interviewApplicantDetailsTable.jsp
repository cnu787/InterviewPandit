...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.pageClick').click(function(e) {
			...
			$('.applicant_interviews').load("getDashboardReport", {
	...
	function cancelMyInterview(intViewId, tmiID, userid) {
		...
		<table>
			...
			<c:forEach var="appList" items="${userDetails}">
				<tr>
					...
					<td><c:if test="${appList.status eq 1}">
										Booked</c:if> <c:if test="${appList.status eq 2}">
										Closed</c:if> <c:if test="${appList.status eq 3}">
										Cancelled </c:if>
										<c:if test="${appList.status eq 4}">
										Payment Cancelled</c:if>
										<c:if test="${appList.status eq 5}">
										Payment Failure</c:if>
										</td>
					<td><c:if test="${appList.status eq 1}">
							<a
								href="javascript:cancelMyInterview('${appList.interviewid}','${appList.interviewtmiid}','${appList.userid}');">Cancel
								Booking</a>
						</c:if></td>
				</tr>
			</c:forEach>
		...
		<div class="modal" id="mycancelInterViewModel" tabindex="-1"
			...
			<div class="modal-dialog mymodal-dialog">
				<div class="modal-content">
					...
					<input type="hidden" name="dashboard" id="dashboard" value="${dashboardId}">
					<input type="hidden" name="enterString" id="enterString" value="${enterString}">
						<div class="row">
